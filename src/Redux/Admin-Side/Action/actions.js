...
export const registerInitiate = (email, password, isAdmin) => {
    ...
    return async function (dispatch) {
        ...
        await axios.post('https://identitytoolkit.googleapis.com/v1/accounts:signUp?key=AIzaSyAh_KBa1xNr-AGIYJEt7RsK1S9M9HZZomk', data).then(async (user) => {
            if (isAdmin) {
...
export const loginInitiate = (email, password) => {
    ...
    return async function (dispatch) {
        const admindata = []
        dispatch(loginStart());
        await axios.post('https://identitytoolkit.googleapis.com/v1/accounts:signInWithPassword?key=AIzaSyAh_KBa1xNr-AGIYJEt7RsK1S9M9HZZomk', userdata).then(async (user) => {
            await axios.get('https://auth-test-f6dd6-default-rtdb.firebaseio.com/Admin.json').then((user1 => {
...
export const Reset_Password_Initialize = (authid, password) => {
    ...
    return async function (dispach) {
        ...
        await axios.post("https://identitytoolkit.googleapis.com/v1/accounts:update?key=AIzaSyAh_KBa1xNr-AGIYJEt7RsK1S9M9HZZomk", userdata).then((user) => {
